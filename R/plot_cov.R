...
#'
#' plot_cov plots the covariance sequence for a number of systems 
#'
#' @param syst_vec vector of systems or StSp object, if only one system
#' @param channel_in integer: SSM input element to plot. Default is `1`.
#' @param channel_out integer: SSM output element to plot. Default is `1`.
#' @param L integer, number of lags to include. 
#'
#' @return the tibble data file 
#'
#' @export
#'
plot_cov <- function(syst_vec, channel_in=1, channel_out=1, L =20 ){
  ...
  for (s in 1:S){
    syst <- syst_v[[s]]
    # calculate IRs 
    IR <- covariance_function.StSp(syst,M=L,plots=0)
  ...
  p <- plot_IR_tibble(data, inp=channel_in, outp=channel_out, main = 'Covariance Sequence', ylab = 'Cv[i,j]')
  return(p) 
